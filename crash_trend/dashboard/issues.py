    ...
    return `
      <div class="issue-accordion" id="issue-acc-${idx}">
        <div class="issue-summary-row" onclick="toggleIssueDetail(${idx})">
          ...
          ${getErrorTypeBadgeHtml(iss.error_type)}
        ...
        <div class="issue-detail-panel" id="issue-detail-${idx}">
          ...
          <button class="btn-copy-prompt" onclick="copyFixPrompt('${esc(iss.issue_id)}', '${esc(iss.platform)}')">
...
function copyFixPrompt(issueId, platform) {
  const app = getCurAppData();
  if (!app) return;
  const snap = getCurPeriodSnapshot();
  const issues = snap?.top_issues || app.top_issues || [];
  const iss = (platform ? issues.find(i => i.issue_id === issueId && i.platform === platform) : null) ||
              (platform ? (app.top_issues || []).find(i => i.issue_id === issueId && i.platform === platform) : null) ||
              issues.find(i => i.issue_id === issueId) ||
              (app.top_issues || []).find(i => i.issue_id === issueId);
  if (!iss) return;
  ...
  const promptLines = [
    ...
    `- 層級: ${iss.error_type} (${iss.error_type === 'FATAL' ? '💥 致命閃退' : (iss.error_type === 'ANR' ? '⏳ 無回應' : '⚠️ 未閃退異常')}) | 優先級: ${iss.priority?.level || "P2"} (分數: ${iss.priority?.score || 0})`,
